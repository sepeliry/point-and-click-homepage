...
class Bookshelf {
  constructor(app, toggleView) {
    ...
    button.x = 0.03 * app.mainScene.width;
    button.y = 0.03 * app.mainScene.height;
    ...
    const width = (12.5 / 100) * app.mainScene.width;
    const height = (30 / 100) * app.mainScene.height;
    ...
    bookshelfMapping.x = app.mainScene.width / 3.72;
    bookshelfMapping.y = app.mainScene.height / 2.5;
    ...
    app.mainScene.addChild(bookshelfMapping);
  ...
  generateBooks(app) {
    ...
    for (let i = 0; i < 4; i++) {
      for (let j = 0; j < 8; j++) {
        ...
        const currentBookImg =
          bookNumber % Math.floor(Math.random() * 10) === 0
            ? this.book2
            : this.book1;
